...
export default function Home() {
  const dashboardCards = [
    ...
    { title: "Intelligence", icon: "/icons/int.svg", href:"/customer/intelligence" },
    { title: "Reports", icon: "/icons/reports.svg", href:"/customer/reports" },
    { title: "News", icon: "/icons/news.svg", href:"/customer/news" },
    { title: "Configure", icon: "/icons/configure.svg", href:"/customer/configure" },
    ...
    <div className="grid -z-20 bg-white bg-opacity-10 backdrop-blur-sm py-10 px-6 rounded-md grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {dashboardCards.map((card, index) => (
        <Link
          key={index}
          href={card.href}
          className="flex flex-col items-center justify-center bg-white p-6 rounded-lg shadow hover:bg-slate-200 transition cursor-pointer"
        >
          <Image
            src={card.icon}
            alt={`${card.title} Icon`}
            ...
            className="mb-4"
          />
          <h3 className="text-lg text-black font-medium">{card.title}</h3>
        </Link>
      ))}
    </div>
  );
