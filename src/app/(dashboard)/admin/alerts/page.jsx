...
const Alerts = () => {
  ...
  const handleSelectAll = (e) => {
    ...
    setSelectedAlertIds(isChecked ? alertUsers.map((user) => user.id) : []);
  };
  ...
  const handleOpenEditUserPopup = () => {
    const selectedAlerts = alertUsers.filter((user) =>
      selectedAlertIds.includes(user.id)
    );
  ...
  const confirmDeleteSelection = () => {
    const remainingUsers = alertUsers.filter(
      (user) => !selectedAlertIds.includes(user.id)
    );
                ...
                currentPageUsers.map((user) => (
                  ...
                  >
                    <td className="td-class text-center">
                      <input
                        type="checkbox"
                        checked={selectedAlertIds.includes(user.id)}
                        onChange={() => handleRowSelection(user.id)}
                        className="cursor-pointer"
                      />
                    </td>
                    <td className="td-class text-center">{user.id}</td>
                    <td className="td-class">{user.username}</td>
                    <td className="td-class">{user.company}</td>
                    <td className="td-class text-center">{user.alerts}</td>
                  </tr>
                ))
              )}
            </tbody>
